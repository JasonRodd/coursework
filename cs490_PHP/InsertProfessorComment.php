...
function InsertAProfessorComment($connection, $post) {
	$returnString = "";
	...
	if($containsUserName && $containsQNUM && $containsExamId && $containsComment){
		...
		$insert = sprintf("INSERT INTO ProfessorComments ( qnum, username, examid, comment ) VALUES ( '%s', '%s', '%s', '%s' )" , $post['qnum'],$post['username'],$post['examid'],$post['comment']);
