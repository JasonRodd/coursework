...
function DeleteAStudentQuestion($connection, $post) {
	$returnString = "";
	...
	if($containsUserName && $containsExamId && $containsQNUM){
		...
		$delete = sprintf("DELETE FROM StudentQuestions WHERE username = '%s' AND examid = '%s' AND qnum = '%s'", $post['username'],$post['examid'], $post['qnum']);
